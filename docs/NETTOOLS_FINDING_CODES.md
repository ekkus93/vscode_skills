...
| `NEIGHBOR_DISCOVERY_PARTIAL` | `warn` | `net.l2_neighbor_discovery` | Neighbor discovery completed with only partial protocol visibility. | Check which neighbor sources are missing and decide whether to gather more switch or controller telemetry. |
| `NO_NEIGHBOR_DATA_AVAILABLE` | `warn` | `net.l2_neighbor_discovery` | No usable L2 neighbor data was available for the requested scope. | Confirm scope identifiers and provider coverage before assuming the segment is empty. |
| `TOPOLOGY_EDGE_INFERRED` | `warn` | `net.topology_map` | One or more topology edges are inferred rather than directly observed. | Review the graph evidence and validate the missing segment with a more direct source if possible. |
| `TOPOLOGY_EDGE_CONFLICT` | `warn` | `net.topology_map` | Topology evidence contains conflicting or unresolved edges. | Compare the disagreeing sources and check for stale telemetry or hidden intermediate devices. |
| `CLIENT_ATTACHMENT_RESOLVED` | `warn` | `net.mac_path_trace` | The client or endpoint attachment point was resolved from MAC-location evidence. | Use the resolved attachment point as the starting context for topology or RF follow-up. |
| `CLIENT_ATTACHMENT_AMBIGUOUS` | `warn` | `net.mac_path_trace` | Multiple candidate attachment points matched the same MAC evidence. | Investigate MAC flaps, roaming, or stale tables before trusting the current attachment. |
| `MAC_PATH_PARTIAL` | `warn` | `net.mac_path_trace` | The attachment point was found, but the full end-to-end path remains incomplete. | Expand the topology evidence around the resolved edge to reconstruct the remaining path. |
| `MAC_NOT_OBSERVED` | `warn` | `net.mac_path_trace` | The requested MAC address was not found in the available switch telemetry. | Confirm the MAC, widen scope, or check whether the endpoint is offline or behind an unmanaged segment. |
| `GATEWAY_PATH_INCOMPLETE` | `warn` | `net.topology_map` | The likely path to the local gateway could not be fully reconstructed. | Gather more adjacency or gateway-interface evidence before drawing a hard conclusion. |
| `MDNS_SERVICES_DISCOVERED` | `warn` | `net.subnet_inventory`, `net.mdns_service_discovery` | Local mDNS or DNS-SD service advertisements were discovered in scope. | Correlate the advertised services with host inventory and confirm whether missing hosts need more discovery. |
| `ACTIVE_SCAN_DISABLED` | `warn` | `net.subnet_inventory` | Active scan options were requested but the required authorization flag was not supplied. | Re-run with explicit authorization only if the subnet scan is permitted by policy. |
| `GATEWAY_LATENCY_HIGH` | `warn` | `net.gateway_health` | First-hop gateway latency is elevated beyond the expected threshold. | Compare first-hop latency with path-probe and local-route evidence to isolate the bottleneck. |
| `GATEWAY_PACKET_LOSS` | `critical` | `net.gateway_health` | The local gateway is dropping traffic above the allowed threshold. | Treat as first-hop degradation and inspect the gateway, uplinks, and local routing immediately. |
| `DUPLICATE_ARP_OWNER` | `warn` | `net.gateway_health`, `net.local_route_anomaly` | Multiple MAC owners were observed for the same local IP or gateway signal. | Inspect ARP ownership, duplicate IP use, and any gateway redundancy or misconfiguration. |
| `POTENTIAL_RF_INTERFERENCE` | `warn` | `net.rf_interference_scan` | AP overlap and channel utilization suggest an RF interference risk. | Review channel assignment and AP density for the affected RF area. |
| `BASELINE_NODE_COUNT_REGRESSION` | `warn` | `net.site_baseline_compare` | The current topology graph contains fewer nodes than the stored baseline. | Compare current topology evidence with recent changes and check whether visibility regressed or devices disappeared. |
| `BASELINE_HOST_VISIBILITY_DROP` | `warn` | `net.site_baseline_compare` | Host visibility is below the stored baseline for the site. | Check whether telemetry sources, local services, or active hosts have dropped unexpectedly. |
| `INTERFACE_SUBNET_MISMATCH` | `warn` | `net.local_route_anomaly` | No gateway interface mapping matched the requested subnet. | Verify interface-to-subnet mapping and confirm the requested scope is on the expected gateway. |
| `ASYMMETRIC_LOCAL_ROUTE` | `warn` | `net.local_route_anomaly` | Multiple default next hops or route choices suggest asymmetric local routing. | Review local route selection, gateway design, and recent routing changes before escalating further. |
